...
Singleton {
  ...
  onShouldRunChanged: {
    if (!shouldRun) return
    initialized = false; // prevent emitting changed signals on startup/reregister
        ...
        try {
          ...
          if (data) {
            if (root.numLockState !== data.numlock) {
              ...
              if (root.initialized) root.numLockChanged();
            ...
            if (root.capsLockState !== data.capslock) {
              ...
              if (root.initialized) root.capsLockChanged();
            ...
            if (root.scrollLockState !== data.scrolllock) {
              ...
              if (root.initialized) root.scrollLockChanged();
  ...
  Timer {
    ...
    onTriggered: {
      if (lockKeysProcess.running) return;
      lockKeysProcess.running = true;
    }
  }
